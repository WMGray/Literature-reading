...
- [ ] 他这个竞争方法85.92%及更低，也是在相同的测试类型上吗？

- [ ] the task of classification of compounds having mixed membership in multiple pathway classes？？指的是一个化合物属于多个pathway？
...
- [ ] 进行特征影响的分析中，第二个模型：具有全局特征输入和前馈网络输出的GCN，这个前馈网络输出是否是GCN嵌入？
...
- 特征重要性分析：
  ...
  - 在以下三个模型中应用Shapley方法：
    ...
    3. GCN嵌入 + 全局特征的RF
       ...
       >
       > 图2.b	条形图的长度给出该特征在所有路径类别上的绝对Shapley值的总和
       >
       > 好像颜色越深，Shapley值越小
